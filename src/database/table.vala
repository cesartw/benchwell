namespace Benchwell {
	namespace Database {
		public class Table : Gtk.Box {
			...
			public signal bool field_changed (Benchwell.Column[] columns);
			public signal bool row_changed (Benchwell.Column[] columns);
			...
			public Table (Benchwell.ApplicationWindow window, Benchwell.DatabaseService service) {
				...
				if (Config.settings.db_edit_panel) {
					var edit_panel_sw = build_edit_panel ();
					edit_panel_sw.set_size_request (200, -1);
					table_edit_paned.pack2 (edit_panel_sw, false, false);
				}
				...
				if (Config.settings.db_edit_panel) {
					edit_panel.save.connect ((columns, iter) => {
						//for (var i = 0; i < columns.length; i++) {
							//print (@"$(columns[i].coldef.name): $(columns[i].val)\n");
						//}

						var c_columns = new Benchwell.Column[service.columns.length + columns.length];
						GLib.Value val;
						for (var i = 0; i < service.columns.length; i++) {
							store.get_value (iter, i, out val);
							c_columns[i] = new Benchwell.Column ();
							c_columns[i].coldef = service.columns[i];
							c_columns[i].val = val.get_string ();
						};

						for (var i = service.columns.length; i < service.columns.length+columns.length; i++) {
							c_columns[i] = columns[i - service.columns.length];
						}

						row_changed (c_columns);
					});
				}
			}
			...
			private void on_selection_changed () {
				...
				if (Config.settings.db_edit_panel) {
					...
					table.get_selection ().selected_foreach ((model, path, i) => {
						if (iter != null) {
							return;
						}
						iter = i;
					...
					edit_panel.set_record (data, iter);
				}
				show_edit_panel ();
			}
			...
			public void hide_edit_panel () {
				if (!Config.settings.db_edit_panel) {
					return;
				}
				edit_panel.get_parent ().hide ();
				edit_panel.get_parent ().get_parent().hide ();
				edit_panel.hide ();
			}

			public  void show_edit_panel () {
				if (!Config.settings.db_edit_panel) {
					return;
				}
				edit_panel.get_parent ().show ();
				edit_panel.get_parent ().get_parent().show ();
				edit_panel.show ();
			}
		...
		public class EditPanel : Gtk.Box {
			...
			private Benchwell.Column[] columns;

			public signal void save (owned Benchwell.Column[] columns, owned Gtk.TreeIter iter);
			...
			public void set_record (owned Benchwell.Column[] columns, owned Gtk.TreeIter iter) {
				clear ();
				this.iter = iter;
				...
				pack_end (btn_save, false, false, 5);
			}
		...
		public class TableModel : Gtk.ListStore {
			public TableModel.newv () {
				Object ();
				set_column_types ({});
			}
		}
